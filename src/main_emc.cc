...
DEFINE_bool(godmode, false, "Should \"Godmode\" patches be applied");
DEFINE_string(input, "C0000001", "Path of the EMC IPL file to load");
DEFINE_string(keys, "keys.json", "Path of the key file to load");
DEFINE_string(output, "C0000001.modified", "Path to save the output EAP KBL file");
...
int main(int argc, char *argv[]) {
  ...
  if (FLAGS_input != "C0000001" && FLAGS_output == "C0000001.modified") {
  ...
  if (s_Encrypt) {
    if (s_Godmode) {
      LOG(INFO) << "Applying \"Godmode\" patches...";
      patch::Pattern(&s_InputData[0], s_InputData.size(), "\x03\x00\xFD\x00", 4, "\x0F", 1);
      patch::Pattern(&s_InputData[0], s_InputData.size(), "\x07\x00\xFD\x00", 4, "\x0F", 1);
    }
    if (!emc::Encrypt(&s_InputData[0], s_InputLen, s_SouthbridgeRevision, s_OutputData)) {
      LOG(FATAL) << "Failed to encrypt EMC IPL";
      return 1;
    }
  } else {
    if (!emc::Decrypt(&s_InputData[0], s_InputLen, s_OutputData)) {
      LOG(FATAL) << "Failed to decrypt EMC IPL";
      return 1;
    }
    if (s_Godmode) {
      LOG(INFO) << "Applying \"Godmode\" patches...";
      patch::Pattern(&s_OutputData[0], s_OutputData.size(), "\x03\x00\xFD\x00", 4, "\x0F", 1);
      patch::Pattern(&s_OutputData[0], s_OutputData.size(), "\x07\x00\xFD\x00", 4, "\x0F", 1);
    }
  }

  if (s_OutputData.size() == 0) {
    LOG(FATAL) << "Failed to modify EMC IPL";
    return 1;
  }
